...
$$T=\frac{\hat{\beta_{age}}-E(\hat{\beta_{age})}{\sqrt{Var(\hat\beta_{age})}}$$
...
**Q8:**
The prediction intervalis very broad and exceeds the value for oth

```{r,eval=TRUE}
new = data.frame(Age=16, Htcm=170, Gender="M", Smoke=0)
#best guess, hjemmekok: 
Ynew = summary(modelA)$coeff[1,1]+summary(modelA)$coeff[2,1]*16+summary(modelA)$coeff[3.1]*170+summary(modelA)$coeff[4,1]*1+summary(modelA)$coeff[5,1]*1
Ynew
...
**Q16:** 
$$E(\beta)=E[({\bf X}^T{\bf X})^{-1}{\bf X}^T{\bf Y}]=({\bf X}^T{\bf X})^{-1}{\bf X}^TE(\bf{Y})=({\bf X}^T{\bf X})^{-1}({\bf X}^TX)\beta=I\beta=\beta$$

$$Cov(\hat\beta)=Cov(({\bf X}^T{\bf X})^{-1}{\bf X}^T{\bf Y})=({\bf X}^T{\bf X})^{-1}{\bf X}^TCov({\bf Y})[({\bf X}^T{\bf X})^{-1}{\bf X}^T]^T=\sigma^2({\bf X}^T{\bf X})^{-1}{\bf X}^T\bf{X}[({\bf X}^T{\bf X})^{-1}]^T=\sigma^2(\bf{X^TX})^{-1}$$
**Q17:** 
$$\text{E}(\hat{f}({\bf x}_0))=\text{E}(\bf{x}_0^T \hat{\boldsymbol\beta})=\bf{x}_0^T E(\hat{\boldsymbol\beta})=\bf{x}_0^T\boldsymbol\beta=f(x_0)$$
$$\text{Var}(\hat{f}({\bf x}_0))=\text{Var}({\bf x}_0^T\boldsymbol{\hat\beta}))=\text{Var}(\hat\beta_0)+(x_{01}^2\text{Var}(\hat\beta_1))+...+(x_{0p}^2\text{Var}(\hat\beta_p))={\bf x}_0^T\circ{\bf x}_0^T\text{diag(Cov}(\boldsymbol{\hat\beta}))=\sigma^2\sum_{i=1}^{p+1} {\bf x}_ø^{T2}A_{jj}
$$ 

**Q18:** 
$$\text{E}[(Y_0-\hat{f}({\bf x}_0))^2]=[\text{E}(\hat{f}({\bf x}_0)-f({\bf x}_0)]^2+\text{Var}(\hat{f}({\bf x}_0) ) + \text{Var}(\varepsilon)=\text{E}[(Y_0-\hat{f}({\bf x}_0))^2]=\text{E}[(Y_0^2-2Y_o\hat{f}({\bf x}_0)+\hat{f}({\bf x}_0)^2]=\text{Var}(Y_0)+\text{E}(Y_0)^2-2 \text{E}(Y_0) \text{E}(\hat{f}(\bf x_0))+\text{Var}(\hat{f}(x_0))+\text{E}(\hat{f}(x_0))^2
=\text{Var}(\varepsilon)+\text{Var}(\hat{f}(\bf x_0))=\sigma^2\sum_{i=1}^{p+1} \bf{x}_ø^{T2}A_{jj}+\text{Var}(\varepsilon)+0
$$
...
$$\text{Cov}(\widetilde{\boldsymbol \beta})=\text{Cov}(({\bf X}^T{\bf X}+\lambda {\bf I})^{-1}{\bf X}^T{\bf Y})=({\bf X}^T{\bf X}+\lambda {\bf I})^{-1}{\bf X}^T\text{Cov}({\bf Y})[({\bf X}^T{\bf X}+\lambda {\bf I})^{-1}{\bf X}^T]^T=\sigma^2({\bf X}^T{\bf X}+\lambda {\bf I})^{-1}{\bf X}^T{\bf X}({\bf X}^T{\bf X}+\lambda {\bf I})^{-1})=\sigma^2\bf C
$$
**Q20:** 
